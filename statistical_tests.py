    ...
    for i in range(m):
        if adjusted_p_values[i] <= alpha:
            significant_results.append(
                (sorted_indices[i], adjusted_p_values[i]))
...
def friedman_test(data):
    ...
    """
    mae_values_all_configs = [[fold["mae"]
                               for fold in config["folds"]] for config in data]
...
def main(filename, dataset):
    ...
    config_without_fss = [
        config for config in data if "100%" in config["config"]][0]

    data = sorted(
        data[:-1], key=lambda x: int(x["config"].split("%")[0].split(" ")[1]), reverse=True
    ) + [data[-1]]
    ...
    if p_value < 0.05:
        ...
        for config in data:
            ...
            different, wilcoxon_p_value = is_statistically_different(
                config, config_without_fss)
        ...
        for config_name, adjusted_p in zip(config_names, adjusted_p_values):
            if adjusted_p < 0.05:
                print(
                    f"Config: {config_name}, Significant (Adjusted p-value: {adjusted_p:.4f})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="perform Friedman test, Wilcoxon tests, and compute A12")
